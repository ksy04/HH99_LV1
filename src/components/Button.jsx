...
// children: 이 컴포넌트가 감싸는 요소(여기서는 버튼) 안에 포함된 JSX나 텍스트를 의미
const Button = ({ clickAddButtonHandler, children }) => {
  // 이 컴포넌트는 <button> 요소를 반환합니다.
  // onClick 속성에는 clickAddButtonHandler 함수가 전달되어, 버튼이 클릭될 때 이 함수가 호출됩니다.
  // 버튼 내부의 내용은 {children}을 통해 동적으로 처리됩니다. 이는 컴포넌트가 감싸는 부분에 어떠한 JSX나 텍스트가 들어가도록 하는 역할을 합니다.
